...
class total_ballistic:
    def __init__(self, model, observer, horizon_length, measurement_lapse, model_pen, opt_method = 'Newton LS'):
    ...
    def slide_window(self, last_y):
        ...
        self.vars[0:7] = self.m.f(self.vars[0:7])
        self.vars[6:self.N+5] = self.vars[7:self.N+6]
        self.x_apriori = self.vars[0:7]
    ...
    def cost_function(self, x):
        ...
        for i in range(0, self.N):
            J = J + 0.5*pow(LA.norm(np.matmul(self.reg1, (self.y[i] - self.o.h(x_iplus1, 'off')))), 2) + 0.5*(self.mu2*(x[7+i] - x_iplus1[6]))**2
            for j in range(self.inter_steps):
                x_iplus1 = self.m.f(x_iplus1, 'off')
            x_iplus1[6] = x[7+i]
        J = J + 0.5*pow(LA.norm(np.matmul(self.reg1, (self.y[self.N] - self.o.h(x_iplus1, 'off')))), 2)
        return J
    ...
    def gradient(self, x):
        ...
        for i in range(self.N):
            df = self.dfdx(x_i)
            dfdx_i.append(df[0:6, 0:6])
            dfdbeta_i.append(df[0:6, 6])
            dhdx_i.append(self.dh(x_i)[:, 0:6])
        ...
        grad[0:6] = grad[0:6] + np.matmul(np.transpose(dhdx_i[0]), np.matmul(R1, h_i[0]-self.y[0]))
        for i in range(1, self.N+1):
            dfdx_mult = dfdx_i[0] #  for dh/d(rv)_i
            ...
            A = np.matmul(dhdx_i[i], dfdx_mult) #  dh(x_i)/d(r_i, v_i)
            B = np.matmul(R1, h_i[i] - self.y[i])
            C = np.matmul(np.transpose(A), B)
            grad[0:6] = grad[0:6] + C

            for j in range(i):
                dfdx_mult_beta = dfdbeta_i[j]
                for l in range(j+1,i):
                    dfdx_mult_beta = np.matmul(dfdx_i[l],dfdx_mult_beta)
                D = np.matmul(dhdx_i[i], dfdx_mult_beta)  # dh/dbeta_i
                E = np.matmul(np.transpose(D), B)
                grad[6+j] = grad[6+j] + E

        grad[6] = grad[6] + self.mu2*(x[6] - x[7])
        for i in range(1, self.N):
            grad[6+i] = grad[6+i] + self.mu2*(2*x[6+i] - x[5+i] - x[7+i])
    ...
    def hessian(self, x):
        ...
        H[0:6, 0:6] = H[0:6, 0:6] + np.matmul(np.transpose(dhdx_i[0]), np.matmul(R1, dhdx_i[0]))
        for i in range(1, self.N + 1):
            ...
            for j in range(i):
                ...
                for l in range(j+1,i):
                    dfdx_mult_beta = np.matmul(dfdx_i[l],dfdx_mult_beta)
                D = np.matmul(dhdx_i[i], dfdx_mult_beta)  # dh/dbeta_i
                E = np.matmul(np.transpose(D), np.matmul(R1, D))
                H[6+j, 6+j] = H[6+j, 6+j] + E

        H[6, 6] = H[6, 6] + self.mu2
        for i in range(1, self.N):
            H[6+i, 6+i] = H[6+i, 6+i] + 2*self.mu2
