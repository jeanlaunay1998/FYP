...
class Memory:
    ...
    def make_plots(self, real_x, real_beta, y_real, Sk, UKF_states, EKF_states):
        ...
        beta_av_error_EKF.append(np.sum(np.abs(EKF_states[self.N[0]:len(EKF_states), 6] - real_beta[self.N[0]:len(real_beta)]))/len(EKF_states))
    ...
    def make_absolute_plots(self, real_x, real_beta, y_real, Sk, UKF_states, EKF_states):
        sns.set()
        ukf_y = []
        ekf_y = []
        for i in range(len(EKF_states)):
        ...
        for j in range(3):
            y = np.abs(UKF_states[self.N[0]:len(UKF_states), j] - real[self.N[0]:len(Sk), 0, j])
            if j ==0:
                ukf_position = np.power(y, 2)
            else:
                ukf_position = ukf_position+np.power(y,2)
            # ax[j, 1].plot(self.t, y, '-b')
            y = np.abs(EKF_states[self.N[0]:len(EKF_states), j] - real[self.N[0]:len(Sk), 0, j])
            if j ==0:
                ekf_position = np.power(y, 2)
            else:
                ekf_position = ekf_position+np.power(y,2)
            # ax[j, 1].plot(self.t, y, '-r')
        ...
        for i in range(self.size):
            ...
            # ax[2, 0].plot(self.t[self.N[i]-self.N[0]:len(self.t)], self.states[i][:, self.N[i], 2], '--', label=labelstring[i])
            for j in range(3):
                y = np.abs(self.states[i][:, self.N[i], j] - real[self.N[i]:len(Sk), 0, j])
                if j == 0:
                    MHE_position.append(np.power(y, 2))
                else:
                    MHE_position[i] = MHE_position[i] + np.power(y, 2)
                # ax[j, 1].plot(self.t, y, '-b')
                # ax[j, 1].plot(self.t[self.N[i]-self.N[0]:len(self.t)], y, '--')
                # ax[j, 1].set(xlabel='Time (s)', ylabel='Error')
        # handles, labels = ax[2,0].get_legend_handles_labels()
        ...
        for j in range(3):
            y = np.abs(UKF_states[self.N[0]:len(UKF_states), j+3] - real[self.N[0]:len(Sk), 1, j])
            if j ==0:
                ukf_velocity = np.power(y, 2)
            else:
                ukf_velocity = ukf_velocity+np.power(y,2)
            # ax[j, 1].plot(self.t, y, '-b')
            y = np.abs(EKF_states[self.N[0]:len(EKF_states), j+3] - real[self.N[0]:len(Sk), 1, j])
            if j ==0:
                ekf_velocity = np.power(y, 2)
            else:
                ekf_velocity = ekf_velocity+np.power(y,2)
            # ax[j, 1].plot(self.t, y, '-r')
        ...
        for i in range(self.size):
            ...
            # ax[2, 0].plot(self.t[self.N[i]-self.N[0]:len(self.t)], self.states[i][:, self.N[i], 5], '--', label=labelstring[i])
            for j in range(3):
                y = np.abs(self.states[i][:, self.N[i], j+3] - real[self.N[i]:len(Sk), 1, j])
                if j == 0:
                    MHE_velocity.append(np.power(y, 2))
                else:
                    MHE_velocity[i] = MHE_velocity[i] + np.power(y, 2)
                # ax[j, 1].plot(self.t[self.N[i]-self.N[0]:len(self.t)], y, '--')
                # ax[j, 1].set(xlabel='Time (s)', ylabel='Error')
        ...
        ekf_velocity_conv = 0
        ukf_velocity_conv = 0
        for i in range(len(ekf_position)):
            if ekf_position[i] <= position_criterion ** 2:
                ekf_position_conv = ekf_position_conv + 1 / len(ekf_position)
            if ukf_position[i] <= position_criterion ** 2:
                ukf_position_conv = ukf_position_conv + 1 / len(ukf_position)
            if ekf_velocity[i] <= velocity_criterion ** 2:
                ekf_velocity_conv = ekf_velocity_conv + 1 / len(ekf_position)
            if ukf_velocity[i] <= velocity_criterion ** 2:
                ukf_velocity_conv = ukf_velocity_conv + 1 / len(ukf_position)

        MHE_position_conv = []
        MHE_velocity_conv = []
        for j in range(self.size):
            MHE_position_conv.append(0)
            MHE_velocity_conv.append(0)
            for i in range(len(MHE_position[j])):
                if MHE_position[j][i] <= position_criterion ** 2:
                    MHE_position_conv[j] = MHE_position_conv[j] + 1 / len(MHE_position[j])
                if MHE_velocity[j][i] <= velocity_criterion ** 2:
                    MHE_velocity_conv[j] = MHE_velocity_conv[j] + 1 / len(MHE_position[j])

        print("Rate of convergence of ekf and ukf: ")
        print('position:')
        print(ekf_position_conv, ' ; ', ukf_position_conv)
        print('velocity:')
        print(ekf_velocity_conv, ' ; ', ukf_velocity_conv)
        print(" Rate of convergence MHEs: ")
        for i in range(self.size):
            print('position:')
            print(MHE_position_conv[i])
            print('velocity:')
            print(MHE_velocity_conv[i])
        ...
        print("Extended Kalman filter average relative error ; Average root square error:  ")
        print("     - Position: ",  '  ;  ', p_av_error_EKF)
        print("     - Velocity: ",  '  ;  ', v_av_error_EKF)
        print("     - Ballistic coefficient: ", beta_av_error_EKF)

        return_array.append([[p_av_error_EKF, v_av_error_EKF, beta_av_error_EKF], ekf_position_conv, ekf_velocity_conv])
        print("Unscented Kalman filter average relative error ; Average root square error:  ")
        print("     - Position: ",  '  ;  ', p_av_error_UKF)
        print("     - Velocity: ",  '  ;  ', v_av_error_UKF)
        print("     - Ballistic coefficient: ", beta_av_error_UKF)
        return_array.append([[p_av_error_UKF, v_av_error_UKF, beta_av_error_UKF], ukf_position_conv, ukf_velocity_conv])
        ...
        for i in range(self.size):
            ...
            beta_av_MHE.append(np.sum(np.abs(self.states[i][:, self.N[i], 6] - real_beta[self.N[i]:len(real_beta)]))/len(self.states[i]))
            print(labelstring[i], 'average relative error ; Average root square error:  ')
            print("     - Position: ", '  ;  ', p_av_MHE)
            print("     - Velocity: ", '  ;  ', v_av_MHE)
            print("     - Ballistic coefficient: ", beta_av_MHE)
...
# Plot approximation model variables
        ...
        #                                            out=np
#                                            .zeros_like(np.array(self.y_model)[:, 0]), where=np.array(y_real)[self.N:len(y_real), 0]!=0)), 'r')
